/**
 * <lang><zh-CN>BP 版本化 local JSON 数据集的唯一模块入口：为 read provider 与 reservation write provider 提供同一静态输入，不执行网络、文件系统、storage 或动态加载。</zh-CN><en>Sole module entry for the BP versioned local-JSON dataset: supplies the same static input to read provider and reservation-write provider without network, file system, storage, or dynamic loading.</en></lang>
