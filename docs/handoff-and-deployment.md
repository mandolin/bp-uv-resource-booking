...
| --- | --- |
| HIA-uView UI | <code>@hia-uview/ui@0.0.0</code> 来自提交 <code>a1803392b6317e9e0b6a83606d06ce47de5639ba</code>。全局样式由 <code>src/uni.scss</code> 引入，<code>u-*</code> 模板通过受限 Easycom 解析到该 pin 的叶级 SFC；发现页使用其 <code>USearch.searchIcon</code> 搜索装饰，H5 主导航直接使用其图标型 <code>UTabbar</code>。 / <code>@hia-uview/ui@0.0.0</code> comes from commit <code>a1803392b6317e9e0b6a83606d06ce47de5639ba</code>. Global styles enter through <code>src/uni.scss</code>, bounded Easycom resolves <code>u-*</code> templates to leaf SFCs at that pin, Discover uses its <code>USearch.searchIcon</code> decoration, and H5 primary navigation directly uses its icon-capable <code>UTabbar</code>. |
...
- 仓库捆绑经过固定来源、语料、生成链与摘要核验的思源黑体 Regular/Bold 和思源宋体 Bold WOFF 子集；H5 使用同源文件，微信开发工具使用同 bytes 的 CSS data URL，均不请求字体 CDN。子集外字形、未来远端数据和未捆绑的思源等宽仍可降级到 Noto CJK 或 generic family，因此截图只证明当前已审语料与环境，不证明所有设备、文本或正式微信上传包体完全一致。
- The repository bundles Source Han Sans Regular/Bold and Source Han Serif Bold WOFF subsets locked by source, corpus, generation chain, and digest. H5 uses same-origin files, and WeChat DevTools uses CSS data URLs containing the same bytes; neither requests a font CDN. Glyphs outside the subset, future remote data, and the unbundled Source Han Mono role may still fall back to Noto CJK or generic families, so screenshots prove only the reviewed corpus and environment—not identical rendering across every device or text, nor production WeChat upload-package compatibility.
